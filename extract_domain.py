...
def run_hmmsearch(hmm_Version, query_hmmfile, fasta_file, cut_off):
    "Run hmmsearch"
    if hmm_Version == 'v3':
        print "Goes for hmmer3!"
        format = 'hmmsearch3-domtab'
        if cut_off == 'tc':
            command = ["hmmsearch", "--cpu", "4","--cut_tc", "--domtblout", "temp.out", query_hmmfile, fasta_file]
        elif cut_off == 'none':
            command = ["hmmsearch", "--cpu", "4","--domtblout", "temp.out", query_hmmfile, fasta_file]
        else:
            command = ["hmmsearch", "--cpu", "4", "--domE", cut_off, "--domtblout", "temp.out", query_hmmfile, fasta_file]
    else:
        format = 'hmmer2-text'
        print "Goes for hmmer2!"
        if cut_off == 'tc':
            command = ["../hmmer-2.3.2/src/hmmsearch",  "--cut_tc", query_hmmfile, fasta_file]
        elif cut_off == 'none':
            command = ["../hmmer-2.3.2/src/hmmsearch",  query_hmmfile, fasta_file]
        else:
            command = ["../hmmer-2.3.2/src/hmmsearch", "--domE", cut_off, query_hmmfile, fasta_file]


    try:
        out, err, retcode = execute(command)
        if hmm_Version == 'v3':
            out = open("hmmersearch3_reduce.out","r").read()
...
def run_HMMer(hmmversion, hmmfile, fastafile, cut_off):
    ...
    if len(hmmer_results) > 0:
        for cds in hmmer_results.keys():
            ...
            for hit in hmmer_results[cds]:
                ...
                #loc = '-'.join(hit)
                loc = '_'
                domain_name_sort.append("%s%s%s" % (cds_name, loc, domain_ab))
...
#
#         for hit in hmmer_results[cds]:
#             hit_start_sort.append(int(hit.hit_start))
#             domain_sequence = cds_sequence[hit.hit_start:hit.hit_end]
#             loc = '-'.join((str(hit.hit_start), str(hit.hit_end)))
#             domain_name_sort.append("%s|%s|%s" % (cds_name, loc, domain_ab))
#             domain_sequence_sort.append(domain_sequence)
...
if __name__ == "__main__":
    ...
    if any(t == [] for t in vars(args).values()) == False:
        ...
        hmmer_results = run_HMMer(hmmversion, hmmfile, fastafile, cut_off)
