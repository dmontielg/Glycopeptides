...
def run_hmmsearch(hmm_Version, query_hmmfile, fasta_file, cut_off,hmmoutput):
    ...
    if hmm_Version == 'v3':
        ...
        if cut_off == 'tc':
            ...
            command = ["hmmsearch", "--cpu", "4", "--domE", cut_off, "--domtblout", hmmoutput, query_hmmfile, fasta_file]
    else:
        format = 'hmmer2-text'
...
def run_HMMer(hmmversion, hmmfile, fastafile, cut_off, hmmoutput):
    ...
    runresults = run_hmmsearch(hmm_Version= hmmversion,  query_hmmfile=hmmfile, fasta_file=fastafile, cut_off = cut_off, hmmoutput= hmmoutput)
    if hmmversion == 'v3':
        results_by_id = {}
